...
The TUI has one reusable session-picker dialog. It is a centered modal overlay that, while open, captures all keyboard input (it sits above every other input surface, including the program view).
...
- A typeahead search line at the top filters by **dimming, not removing**: non-matching sessions stay in place but render dimmed; matching sessions render bright. An empty query dims nothing.
...
The dialog is opened from two places, distinguished only by what confirming does:

- The `C-x b` keybinding opens it as a **session switcher** — confirming focuses the chosen session in the active window.
- The program view's `@`→session path opens it as a **clip picker** — confirming inserts an `@{session:id}` clip into the program buffer.
...
- `C-x b` with sessions `alpha`, `beta`, and a `Proj` group holding `gamma`: the dialog lists all three bright under their headers; the archived sessions inside `Proj` stay hidden behind a collapsed "N archived" row.
- Typing `al`: `alpha` stays bright and selectable, `beta` dims in place, and `Proj` collapses to its header because none of its sessions match.
...
- In a program, `@` then the `session` category: the same dialog opens; picking `alpha` replaces the `@…` token with `@{session:<id>}` and closes both the dialog and the `@` search.
