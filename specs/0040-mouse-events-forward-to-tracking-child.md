...
- A client-owned overlay open over a pane (session-title menu, program popup) takes mouse priority over the child beneath it. While such an overlay is open the client handles the mouse itself, so the overlay's actions stay clickable even when the pane's harness has grabbed the mouse.
