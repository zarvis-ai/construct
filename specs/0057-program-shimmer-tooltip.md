...
Date: 2026-06-28
Area: protocol
Scope: Program-block shimmer carries a required, concise run-status tooltip that renderers surface on hover.

## Decision

A program block's shimmer state carries a **tooltip**: a concise, ≤10-word string describing why that block is still running (e.g. "Building PR", "Waiting on CI").
...
- **A block that shimmers without a stored tooltip renders a hardcoded fallback** (e.g. "Working…"). This covers optimistic client-side shimmer (created before any agent tooltip exists), legacy/in-flight run state, and a block kept pending across a text-changing edit (`keep_pending`), whose new id carries no tooltip until the agent re-declares it.
...
- The shimmer declaration types and the per-block projection gain an optional tooltip field; the active-run model stores a per-block tooltip map kept in sync with the pending set. These are additive and default-empty, so older payloads and in-flight runs deserialize unchanged and simply present no tooltip (the fallback covers them).
...
- A renderer must treat "shimmering with no stored tooltip" as a normal state and substitute the fallback, never as an error or an empty label.
...
- **Optimistic fallback.** The instant a Run is invoked, the whole region shimmers optimistically before any agent tooltip exists; hovering a block shows "Working…". Once the planning pass lands, hovering shows the real per-block status.
