...
- **The hover requires pointer-enter, not mere position.** A block that starts shimmering while the pointer already happens to rest on it — e.g. a selection-Run context menu that sits adjacent to the selection, leaving the pointer on the block the instant it starts shimmering once the menu closes — must NOT immediately open the tooltip. It opens only once the pointer actually moves onto the block after its shimmer began (a genuine pointer-enter). Moving away and back onto a block that has been shimmering all along still opens it normally, matching platform hover conventions. A surface with no reliable notion of "the pointer moved" (vs. content merely changing under a stationary pointer) is exempt from this requirement, same as the general hover exemption above.
- **The tooltip anchors beside the hovered block, never over it.** When it opens, it's placed on a row adjacent to the block — directly below the block's last on-screen row by default, or above the block's first row when the surface's bottom edge would clip it below — so it never paints over any row of the shimmering text it describes.
...
- A renderer needs a way to tell "the pointer moved onto this cell" apart from "this cell started shimmering while the pointer already sat there", and a way to place the tooltip relative to the block's on-screen row span rather than only the pointer's own cell.
...
- **Pointer-enter / placement.** A user selects text and clicks "Run" in the selection context menu, which sits adjacent to the selection; the pointer is left resting on the block the instant it starts shimmering. No tooltip opens. The user then moves the pointer away and back onto the block: the tooltip now opens, anchored on the row directly below the block rather than over its text.
