...
Updating an Operator widget briefly reveals it in the Matrix-rain panel. The title bar shows the Operator name, current status, and the widget count; when visible, previous/next controls let the user browse the widget stack. The existing Matrix-rain close button continues to hide the rain panel itself. When the widget viewport hides or no Operator widgets exist, the panel returns to Matrix rain.
