...
fn canvas_run_instructions() -> Vec<String> {
    vec![
        ...
        "Smart clips are Markdown-native typed references. The clip_id attribute identifies a specific clip instance, not the target itself; preserve clip_id values when editing existing clips. Session clips (`@{session:<id>}`) that reference a subagent you created may be inserted into the canvas at your discretion even without an explicit user request — they surface live output and are lightweight additions.".to_string(),
