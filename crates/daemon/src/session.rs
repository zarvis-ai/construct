...
fn canvas_run_instructions() -> Vec<String> {
    vec![
        ...
        "Shimmer semantics: shimmer on a block means 'a subagent is actively working on this — outcome still unknown'; no shimmer means 'settled — done, skipped, or no action needed'. Editing a block always clears its shimmer unless you explicitly pass shimmer: true, which re-adds it. Never set shimmer: true on a block that is not actively running.".to_string(),
        "Planning pass — your very first canvas action: before starting any task or creating any subagent, make a construct_canvas_edit that touches every block. For each block that will be handed off to a subagent (outcome unknown), pass shimmer: true so it stays animated while the subagent runs. For every other block (no work needed, already done, or being skipped), omit shimmer so it clears immediately. This single early edit makes the canvas reflect your plan within seconds of the Run trigger instead of after the first task completes.".to_string(),
