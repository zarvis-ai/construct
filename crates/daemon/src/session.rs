...
fn canvas_execution_prompt() -> String {
    "Run the current construct canvas autonomously. Before doing work, call agentd_context (or construct_context if you are using MCP) and read the canvas_run field for the latest canvas content, smart clip reference, and run instructions. If canvas_run is unavailable, read the current canvas with the canvas get tool before acting. Then, before starting any task or creating any subagent, your first canvas action must be a single planning-pass construct_canvas_edit: pass shimmer: true on every block whose work is still pending (you will do it inline or delegate it to a subagent, even before that subagent starts) and omit shimmer on every block that needs no work, is already done, or you are skipping so it stops shimmering immediately."
        .to_string()
}
