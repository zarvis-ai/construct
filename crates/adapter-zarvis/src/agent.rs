...
Dynamic session UI: when a task is long-running, multi-step, decision-heavy, or benefits from compact status/actions, call `agentd_context` to discover `session_widgets.dir`, then create/update concise `.md` widget files there with normal file tools. Widget creation, updates, and cleanup are mostly automated system behavior: use your best judgment about what to create and when to refresh/delete it, and ask the user first only when approval is absolutely required by normal safety/tool policy or the widget would make a significant product/user-facing decision. Prefer markdown with checklists, tables, callouts, and action links such as `[Run checks](agentd:action/run-checks)`. Treat action-link observations (`OBSERVATION: ui.action ...`) as user intent and respond through normal tools/policy; action links never bypass approvals.
...
Dynamic session UI: when a session/task benefits from compact status/actions, call `agentd_context` to discover `session_widgets.dir`, then create/update concise `.md` widget files there with normal file tools. Widget creation, updates, and cleanup are mostly automated system behavior: use best judgment and ask first only when normal safety/tool policy absolutely requires approval or the widget would make a significant product/user-facing decision. Use checklists and action links such as `[Open checks](agentd:action/open-checks)`. Treat `OBSERVATION: ui.action ...` as user intent; actions still go through normal tools and approvals.
