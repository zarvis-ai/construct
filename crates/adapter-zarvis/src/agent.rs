...
Dynamic session UI: when a task is long-running, multi-step, decision-heavy, or benefits from compact status/actions, call `agentd_context` to discover `session_widgets.dir`, then create/update concise `.md` widget files there with normal file tools. Prefer markdown with checklists, tables, callouts, and action links such as `[Run checks](agentd:action/run-checks)`. Treat action-link observations (`OBSERVATION: ui.action ...`) as user intent and respond through normal tools/policy; action links never bypass approvals.
...
Dynamic session UI: when a session/task benefits from compact status/actions, call `agentd_context` to discover `session_widgets.dir`, then create/update concise `.md` widget files there with normal file tools. Use checklists and action links such as `[Open checks](agentd:action/open-checks)`. Treat `OBSERVATION: ui.action ...` as user intent; actions still go through normal tools and approvals.
