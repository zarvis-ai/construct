...
const MEMORY_POLICY: &[&str] = &[
    ...
    "At task end, briefly reflect whether anything durable was learned; if yes, update memory before final response.",
    "Capture failure learning only after the cause and fix are understood; do not memorialize every failed attempt.",
    "Prefer conservative, deduplicated, bounded rewrites of existing sections over append-only growth. Merge overlapping entries and compact long sections into concise bullets.",
