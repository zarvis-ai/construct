...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // agentd fork patch: save scrolled-off lines to scrollback
            // whenever the scroll region's top margin is at the top of the
            // screen (`scroll_top == 0`), matching xterm / xterm.js,
            // instead of only when no region is active. Children like
            // codex pin a footer with a top-anchored DECSTBM region
            // (e.g. `ESC[1;37r`); upstream's `!scroll_region_active()`
            // dropped their entire conversation from scrollback. See
            // specs/0017-pty-scrollback-is-real-terminal-history.md.
    ...
    // agentd fork: no longer used by `scroll_up` (see the patch there),
    // kept to minimize the diff from upstream.
    #[allow(dead_code)]
