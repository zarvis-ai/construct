...
seed_sessions() {
  ...
  sid="$($CLIENT_BIN new --title "welcome shell" shell "" | tr -d '[:space:]')"
  if [[ -n "$sid" ]]; then
    $CLIENT_BIN send "$sid" "printf 'Welcome to isolated construct test env\\nworktree: %s\\n\\n' '$WT'; pwd; ls scripts; echo ready" >/dev/null || true
  fi

  sid="$($CLIENT_BIN new --title "activity shell" shell "" | tr -d '[:space:]')"
  if [[ -n "$sid" ]]; then
  ...
  sid="$($CLIENT_BIN new --title "browser preview prompt" --mode headless smith "Use browser_open to open https://example.com with preview true, then summarize what changed in the UI." | tr -d '[:space:]' || true)"
  if [[ -n "$sid" ]]; then
    :
  fi
